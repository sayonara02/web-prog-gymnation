...
// Create upload middleware instances (local storage)
const uploadPostLocal = multer({
  storage: postStorage,
  ...commonOptions
});
...
// Cloudinary upload middleware (single file)
const uploadPostCloudinary = multer({ 
  fileFilter,
...
// Middleware to determine which upload method to use
const uploadPost = useCloudinary ? uploadPostCloudinary : uploadPostLocal;
const uploadProfile = useCloudinary ? uploadProfileCloudinary : uploadProfileLocal;

// Helper function to get file URL
const getFileUrl = (filename, type) => {
  if (useCloudinary) {
    // Cloudinary URLs are already full URLs returned from upload
    return filename; // In cloudinary mode, filename contains full URL
  } else {
    // Local file URL
